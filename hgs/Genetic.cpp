...
{	
	...
	if (maxIterNonProd == 1) {
		...
		population->bestSolutionRestart.myCostSol.penalizedCost = DBL_MAX;
	}
	clock_t t = clock();
	for (int nbIter = 0 ; (nbIterNonProd < maxIterNonProd || !isFeasible) && (float) ((clock() - t)/CLOCKS_PER_SEC) < timeLimit ; nbIter++)
	{	
		...
		if (nbIter % 100 == 0 && maxIterNonProd > 1) population->managePenalties() ;
		// if (nbIter % 100 == 0) population->printState(nbIter, nbIterNonProd);

		/* FOR TESTS INVOLVING SUCCESSIVE RUNS UNTIL A TIME LIMIT: WE RESET THE ALGORITHM/POPULATION EACH TIME maxIterNonProd IS ATTAINED*/
		// if (timeLimit != INT_MAX && (nbIterNonProd == maxIterNonProd || isFeasible))
		// {
		// 	population->restart();
		// 	nbIterNonProd = 1;
		// }
